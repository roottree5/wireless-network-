...
**PWA 구조 소개**
저희 프로젝트는 웹 기반 애플리케이션으로, React로 구현한 프론트엔드와 Node.js로 구현한 백엔드로 구성.
프론트엔드에서는 사용자가 직접 보고 조작하는 인터페이스를, 백엔드에서는 데이터베이스 연동과 도어락 제어를 담당
...
- 중앙의 큰 버튼으로 도어락을 열고 닫을 수 있음.
- 빨간색은 잠김, 초록색은 열림 상태를 나타냄.
- 버튼을 누르면 MQTT로 라즈베리파이에 제어 명령을 전송함.
...
- RFID나 지문 인증 시도가 실시간으로 표시됨
- 성공한 경우 초록색, 실패한 경우 빨간색으로 구분됨
- 시간순으로 정렬되어 최신 기록을 바로 확인할 수 있음
...
**출입 기록은 5초마다, 도어 상태는 1초마다 자동으로 갱신되어 실시간으로 정보를 제공.**
